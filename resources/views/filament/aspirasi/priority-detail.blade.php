@php
    ...
    // Map label -> style badge (sesuaikan dengan enum kamu)
    $priorityStyles = match (strtolower((string) $priorityLabel)) {
        'tinggi', 'high' => 'bg-red-50 text-red-700 ring-red-200',
        'sedang', 'medium' => 'bg-yellow-50 text-yellow-700 ring-yellow-200',
        'rendah', 'low' => 'bg-green-50 text-green-700 ring-green-200',
        default => 'bg-gray-50 text-gray-700 ring-gray-200',
    };

    $confidence = is_null($prioritas?->confidence) ? null : (float) $prioritas->confidence;
    // confidence bisa 0..1 atau 0..100, kita normalkan
    $confidencePercent = is_null($confidence)
        ? null
        : ($confidence <= 1 ? $confidence * 100 : $confidence);

    $confidenceText = is_null($confidence)
        ? '-'
        : ($confidence <= 1 ? number_format($confidence, 3) : number_format($confidence / 100, 3));
    ...
    @if (! $prioritas)
        <div class="rounded-lg border border-gray-200 bg-gray-50 p-4">
            ...
            <div class="mt-1 text-sm text-gray-600">
                Prioritas belum diklasifikasi untuk aspirasi ini.
            </div>
        </div>
    @else
        {{-- Top summary --}}
        ...
        <div class="grid gap-4 sm:grid-cols-2">
            ...
            <div class="rounded-lg border border-gray-200 bg-white p-4">
                ...
                <div class="mt-1 text-[11px] text-gray-500">
                    {{ is_null($confidencePercent) ? '-' : ((int) round($confidencePercent)) . '%' }}
                </div>
            </div>
